...
/**
 * 红黑树
 * https://github.com/TheAlgorithms/Java/blob/master/DataStructures/Trees/RedBlackBST.java
 *
 * insert() 构建二叉搜索树，且完成左旋、右旋进行调整；
 * delete() 删除节点后，进行调整；
 */
public class RedBlackBST {
    ...
    public static void main(String[] args) {
        ...
        int[] arr = {1, 2, 6, 0, 44, 11, 9};

        for (int i = 0; i < arr.length; i++) {
            Node node = redBlackBST.new Node(arr[i]);
            redBlackBST.insert(node);
        }

        redBlackBST.printTree(redBlackBST.root);
        System.out.println();

        redBlackBST.delete(redBlackBST.new Node(6));
        redBlackBST.printTree(redBlackBST.root);
    }
}
